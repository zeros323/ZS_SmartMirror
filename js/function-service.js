var FUNCTIONSERVICE = {
	...
	slideshowFamilyPhoto : function($scope) {
		console.log("가족 사진 보이기");
		if(responsiveVoice.voiceSupport()) {
	          responsiveVoice.speak("네. 가족사진을 감상하시기 바랍니다.","Korean Female");
		}
		
 		$('.cycle-slideshow').cycle('reinit');
 		
 		var fs = require('fs');
 		fs.readdir('photos/', function (err, files) { // '/' denotes the root folder
 		  if (err) throw err;
	
 		   files.forEach( function (file) {
 		     fs.lstat('/'+file, function(err, stats) {
 		       if (!err && stats.isDirectory()) { //conditing for identifying folders
 		         //$('div#slideshowphoto').append('<li class="folder">'+file+'</li>');
 		       }
 		       else{
 		    	//$('div#slideshowphoto').append('<img src="photos/'+file+'"/>');
 		    	//$('div#slideshowphoto').addClass("cycle-slideshow");
 		    	$('.cycle-slideshow').cycle('add', '<img src="photos/'+file+'" id="'+file+'">');
		...
		$scope.focus = "slideshowfamilyphoto";
	},
